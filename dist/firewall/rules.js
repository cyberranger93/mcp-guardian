...
function getArgValues(args) {
    return Object.values(args).map((v) => typeof v === 'string' ? v : JSON.stringify(v));
}
function getArgValuesRaw(args) {
    return Object.values(args).map((v) => typeof v === 'string' ? v : JSON.stringify(v));
}
// Rate-limit state: toolName → timestamps of recent calls
const callTimestamps = new Map();
function isRateLimited(toolName, windowMs, limit) {
    const now = Date.now();
...
export const builtinRules = [
    {
        ...
        description: 'Blocks tool calls to tools with names suggesting OS-level code execution (exec, shell, system, run_command, etc.).',
        severity: 'critical',
        match(call) {
    ...
    },
    {
        ...
        description: 'Blocks tool call arguments containing phrases used in prompt injection attacks like "ignore previous instructions" or "override".',
        severity: 'critical',
        match(call) {
    ...
    },
    {
        ...
        description: 'Detects base64-encoded strings in arguments that, when decoded, contain instruction-like content.',
        severity: 'high',
        match(call) {
            ...
            for (const val of getArgValuesRaw(call.args)) {
                if (base64Pattern.test(val.trim())) {
                    try {
                        ...
                        if (instructionKeywords.test(decoded))
                            return true;
                    }
                    catch {
    ...
    },
    {
        ...
        description: 'Blocks email or webhook tool calls where the recipient looks like data exfiltration (free email domains, suspicious TLDs, or unexpected external endpoints).',
        severity: 'high',
        match(call) {
            const isEmailOrWebhookTool = /\b(send_email|email|webhook|notify|post_to|http_post|slack|discord)\b/i.test(call.toolName);
            if (!isEmailOrWebhookTool)
                return false;
            const raw = stringifyArgs(call.args);
    ...
    },
    {
        ...
        description: 'Blocks file operation tool calls that reference sensitive system paths such as /etc/passwd, .ssh/, .env, or credential stores.',
        severity: 'critical',
        match(call) {
            const raw = stringifyArgs(call.args);
    ...
    },
    {
        ...
        description: 'Flags when more than 10 calls to the same tool occur within a 60-second window — a pattern consistent with automated abuse or runaway loops.',
        severity: 'medium',
        match(call) {
    ...
    },
    {
        ...
        description: 'Blocks tool call arguments containing destructive or injection SQL patterns such as DROP TABLE, DELETE FROM, or inline comment sequences.',
        severity: 'high',
        match(call) {
            const raw = stringifyArgs(call.args);
            const sqlPatterns = [
    ...
    },
    {
        ...
        description: 'Blocks tool calls where the surrounding LLM context contains known jailbreak or adversarial role-play patterns.',
        severity: 'critical',
        match(call) {
    ...
    },
    {
        ...
        description: 'Blocks tool calls that target plain HTTP (non-HTTPS) endpoints, which are susceptible to interception and MITM attacks.',
        severity: 'medium',
        match(call) {
            const raw = stringifyArgs(call.args);
    ...
    },
    {
        ...
        description: 'Blocks tool calls containing PEM-encoded private keys or similar cryptographic secrets being passed as arguments.',
        severity: 'critical',
        match(call) {
            const raw = stringifyArgs(call.args);
            const keyPatterns = [
    ...
    },
    {
        ...
        description: 'Detects when a JWT token is passed as an argument to a tool that does not appear to handle authentication.',
        severity: 'high',
        match(call) {
    ...
    },
    {
        ...
        description: 'Blocks tool call arguments that reference other tool names, which can be used to chain unintended tool invocations.',
        severity: 'high',
        match(call) {
            const raw = stringifyArgs(call.args);
    ...
    },
    {
        ...
        description: 'Blocks tool calls to tools that accept parameters named "code", "script", "eval", or "execute" — high-risk patterns for remote code execution.',
        severity: 'critical',
        match(call) {
    ...
    },
    {
        ...
        description: 'Flags tool calls where any single argument exceeds 10,000 characters — a common vector for hiding malicious instructions inside large payloads.',
        severity: 'medium',
        match(call) {
    ...
    },
    {
        ...
        description: 'Blocks tool call arguments containing Unicode bidirectional control characters (U+202A–U+202E, U+2066–U+2069) used to visually spoof content.',
        severity: 'high',
        match(call) {

...
program
  ...
  .option(
    '-p, --policy <policy>',
    'Policy to apply: strict | standard | audit | path/to/policy.json',
    'standard',
  )
  .option(
    '--fail-on <severity>',
    'Exit with code 1 if any finding meets or exceeds this severity: critical | high | medium',
    'high',
  )
  .option(
    '--save <path>',
    'Save the raw scan JSON to a file (useful for later rendering or SARIF upload)',
  )
...
program
  ...
  .action(
    (
      serverPath: string,
      opts: { output: string; policy: string; color: boolean },
    ) => {
      const outputFormat = validateOutputFormat(opts.output);
...
program
  ...
  .option('--no-color', 'Disable color output')
  .action(
    (reportPath: string, opts: { output: string; color: boolean }) => {
      const outputFormat = validateOutputFormat(opts.output);
...
function runAndPrint(serverPath: string, outputFormat: OutputFormat, color: boolean): void {
  try {
...
function validateOutputFormat(val: string): OutputFormat {
  if (val === 'json' || val === 'table' || val === 'sarif') return val;
  console.error(`Invalid output format "${val}". Must be one of: json, table, sarif`);
  process.exit(2);
}

function validateFailThreshold(val: string): FailThreshold {
  if (val === 'critical' || val === 'high' || val === 'medium') return val;

import type { ScanReport, Severity, OutputFormat } from '../types.js';
...
interface SarifResult {
  ...
  properties: { severity: Severity; remediation: string };
}
...
export function formatTable(report: ScanReport, useColor: boolean = true): string {
  const lines: string[] = [];
  ...
  for (const [severity, count] of Object.entries(counts)) {
    lines.push(`    ${severityColor(severity as Severity)}${severity.toUpperCase()}${c.reset}: ${count}`);
  }
  lines.push('');

 ...
 */
export function scanPermissions(permissions: MCPPermissions | undefined): ScanFinding[] {
  ...
  if (!permissions || Object.keys(permissions).length === 0) {
    findings.push({
      ...
      description:
        'The server manifest does not declare any permission scopes. Without explicit permissions, the actual access granted is opaque to auditors and tooling.',
      remediation:
        'Add a "permissions" block that explicitly declares filesystem, network, and scope permissions. Apply least-privilege principles.',
  ...
  const broadWritePaths = writePaths.filter((p) =>
    p === '/' || p === '*' || p === '**' || p === '.' || p === './' || p === '/home',
  );
  if (broadWritePaths.length > 0) {
    findings.push({
      ...
      description:
        `The server grants write access to overly broad paths: ${broadWritePaths.join(', ')}. ` +
        'This could allow the server to modify arbitrary files on the host system, including system files and credential stores.',
      remediation:
        'Restrict filesystem write permissions to the specific directories the server needs. Use absolute paths to a dedicated working directory.',
  ...
  if (networkPerms?.allowAllHosts === true) {
    findings.push({
      ...
      description:
        'The server is configured with "allowAllHosts: true", granting unrestricted outbound network access. ' +
        'This enables data exfiltration to any external host.',
      remediation:
        'Replace "allowAllHosts: true" with an explicit "allowedHosts" list containing only the required external hosts. ' +
    ...
    findings.push({
      ...
      description:
        'The server declares network permissions but does not specify an "allowedHosts" allow-list. ' +
        'Without an explicit allow-list, network egress policy is undefined.',
      remediation:
        'Define an "allowedHosts" array listing every external host this server legitimately needs to contact.',
  ...
  const broadScopes = scopes.filter(
    (s) =>
      s === '*' ||
      s === 'admin' ||
      s === 'superuser' ||
  ...
  if (broadScopes.length > 0) {
    findings.push({
      ...
      description:
        `The following scopes grant elevated or broad access: ${broadScopes.join(', ')}. ` +
        'Over-privileged scopes violate the principle of least privilege and increase blast radius if the server is compromised.',
      remediation:
        'Audit each scope and remove any that are not strictly required. Prefer read-only scopes over write, and specific resource scopes over wildcards.',
